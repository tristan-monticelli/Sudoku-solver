...
def _brute_force(grid, rows, cols, boxes, callback) -> bool:
    """Ordre fixe : prend la première case vide (gauche→droite, haut→bas)."""
    for r in range(9):
        for c in range(9):
            if grid[r][c] == 0:
                br, bc = r // 3, c // 3
                for n in range(1, 10):
                    ...
                    if callback:
                        callback(grid)
                    if _brute_force(grid, rows, cols, boxes, callback):
                        return True
                    grid[r][c] = 0
                    rows[r].discard(n)
                    cols[c].discard(n)
...
def solve_brute_force(sudoku_grid: SudokyGrid, callback=None) -> tuple:
    """
    Résout un Sudoku par force brute (ordre fixe, sans heuristique).
    ...
    solved = _brute_force(grid, rows, cols, boxes, callback)
