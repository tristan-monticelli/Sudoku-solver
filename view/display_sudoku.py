...
class DisplaySudoku:
    ...
    def draw_countdown(self, grid_bt, grid_bf, initial_mask, current_label, elapsed):
        ...
        if val > 0:
            ...
            n = self.f_big.render(str(val), True, (231, 76, 60))
    ...
    def draw_stats(self, mx, my, click, puzzle_categories, history):
        ...
        for idx, (cat, _files) in enumerate(puzzle_categories):
            ...
            self.screen.blit(self.f_stat.render(cat, True, WHITE), (col_x[0], y0))
            self.screen.blit(
            ...
            for bx, avg, color in [
                ...
                if avg is not None and max_val > 0:
                    ...
                    self.screen.blit(vt, vt.get_rect(
                        centerx=bx + bar_w // 2, bottom=chart_bottom - bar_h - 2))
                else:
        ...
        for color, label, offset in [(CYAN, "Backtracking", -100),
            ...
            pygame.draw.rect(self.screen, color, (start_x, leg_y + 2, sq_sz, sq_sz))
        ...
        self.screen.blit(self.f_stat_h.render("Moy. globale :", True, PURPLE),
                         (cx - 390, y_glob + 8))
        self.screen.blit(
